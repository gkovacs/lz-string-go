...
// replace 45 (-) with 47 (/) and replace 36 ($) with 61 (=)
var keyStrBase64 map[byte]int = map[byte]int{74: 9, 78: 13, 83: 18, 36: 64, 109: 38, 114: 43, 116: 47, 101: 30, 45: 63, 73: 8, 81: 16, 113: 42, 49: 53, 50: 54, 54: 58, 76: 11, 100: 29, 107: 61, 121: 50, 77: 12, 89: 24, 105: 34, 66: 1, 69: 4, 85: 20, 48: 52, 119: 48, 117: 46, 120: 49, 52: 56, 56: 60, 110: 39, 112: 41, 70: 5, 71: 6, 79: 14, 88: 23, 97: 26, 102: 31, 103: 32, 67: 2, 118: 47, 65: 0, 68: 3, 72: 7, 108: 37, 51: 55, 57: 61, 82: 17, 90: 25, 98: 27, 115: 44, 122: 51, 53: 57, 86: 21, 106: 35, 111: 40, 55: 59, 43: 62, 75: 10, 80: 15, 84: 19, 87: 22, 99: 28, 104: 33}
...
func getBaseValueBase64(char byte) int {
	return keyStrBase64[char]
}

// Input is composed of ASCII characters, so accessing it by array has no UTF-8 pb.
func readBitsBase64(nb int, data *dataStruct) int {
	result := 0
	power := 1
	for i := 0; i < nb; i++ {
		...
		if data.position == 0 {
			data.position = 32
			data.val = getBaseValueBase64(data.input[data.index])
			data.index += 1
		}
		if respB > 0 {
			result |= power
...
func getStringBase64(last string, data *dataStruct) (string, bool, error) {
	c := readBitsBase64(data.numBits, data)
	switch c {
	case 0:
		str := string(readBitsBase64(8, data))
		appendValue(data, str)
		return str, false, nil
	case 1:
		str := string(readBitsBase64(16, data))
		appendValue(data, str)
		return str, false, nil
	case 2:
		return "", true, nil
...
func DecompressFromBase64(input string) (string, error) {
	data := dataStruct{input, getBaseValueBase64(input[0]), 32, 1, []string{"0", "1", "2"}, 5, 2}

	result, isEnd, err := getStringBase64("", &data)
	if err != nil || isEnd {
		return result, err
	}
	last := result
	data.numBits += 1
	for {
		str, isEnd, err := getStringBase64(last, &data)
		if err != nil || isEnd {
			return result, err
		}
